...
async function fetchUserData(userId: string) {
  const [profileRes, rolesRes, participantRes, settingsRes] = await Promise.all([
...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  initialize: async () => {
    ...
    if (session?.user) {
      const { profile, roles, ctfParticipant, ctfSettings } = await fetchUserData(session.user.id);
      set({
        user: session.user,
        profile,
        roles,
    ...
    supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if (user) {
        ...
        setTimeout(async () => {
          const { profile, roles, ctfParticipant, ctfSettings } = await fetchUserData(user.id);
          set({ user, profile, roles, ctfParticipant, ctfSettings, loading: false });
        }, 0);
      } else {
        set({
          user: null,
